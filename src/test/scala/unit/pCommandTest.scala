...
class pCommandTest extends AnyFlatSpec with Matchers {
  "A read scroll command" should "imply all the facts implied by the existence of the scroll" in {
    ...
    command._implications should contain(ScrollKnowledge("abcde", ScrollPower.ENCHANT_ARMOR))
  }
  ...
  "A read scroll command" should "know that the scroll has been consumed" in {
    ...
    command._implications should contain(InSlot(Slot.A, Some(Scroll(1, "abcde", ScrollPower.TELEPORTATION))))
    command._implications should not contain InSlot(Slot.A, Some(Scroll(2, "abcde", ScrollPower.TELEPORTATION)))
  }
}
